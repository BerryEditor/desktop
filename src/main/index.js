...
const createEditorWindow = () => {
  ...
  window.webContents.on("will-prevent-unload", (e) => {
    const choice = dialog.showMessageBoxSync(window, {
      title: APP_NAME,
      type: "info",
      buttons: [getTranslation("unload.stay"), getTranslation("unload.leave")],
      cancelId: 0,
      defaultId: 0,
...
const createPackagerWindow = (editorWebContents) => {
  const window = createWindow(
    `${getURL("packager")}&editor_id=${editorWebContents.id}`,
    {
      title: PACKAGER_NAME,
      width: 700,
      height: 700,
...
const createExtensionsWindow = (editorWebContents) => {
  const window = createWindow(
    `tw-extensions://./index.html?editor_id=${editorWebContents.id}`,
    {
...
const getLastAccessedDirectory = () =>
  store.get("last_accessed_directory") || "";
const setLastAccessedFile = (filePath) =>
  store.set("last_accessed_directory", pathUtil.dirname(filePath));
...
ipcMain.handle("show-save-dialog", async (event, options) => {
  const result = await dialog.showSaveDialog(
    BrowserWindow.fromWebContents(event.sender),
    {
      ...
      defaultPath: pathUtil.join(
        getLastAccessedDirectory(),
...
ipcMain.handle("get-packager-html", async () => {
  const compressed = await readFile(
    pathUtil.join(staticDir, "packager.html.br")
  );
...
ipcMain.on("alert", (event, message) => {
  dialog.showMessageBoxSync(BrowserWindow.fromWebContents(event.sender), {
    title: APP_NAME,
    message: "" + message,
    buttons: [getTranslation("prompt.ok")],
    noLink: true,
  });
...
ipcMain.on("confirm", (event, message) => {
  const result =
    dialog.showMessageBoxSync(BrowserWindow.fromWebContents(event.sender), {
      title: APP_NAME,
      message: "" + message,
      buttons: [getTranslation("prompt.ok"), getTranslation("prompt.cancel")],
      defaultId: 0,
      cancelId: 1,
      noLink: true,
    }) === 0;
...
const requestURLAsArrayBuffer = (url) =>
...
app.on("session-created", (session) => {
  ...
  session.webRequest.onBeforeRequest((details, callback) => {
    ...
    if (destinationURL.hostname.startsWith("extensions.tinypatch.ml")) {
    ...
    if (
      destinationURL.protocol === "file:" &&
      !destinationURL.href.startsWith(rootFileURL)
    ) {
      return callback({
        cancel: true,
      });
...
whenBackgroundThrottlingChanged((backgroundThrottlingEnabled) => {
  for (const editorWindow of editorWindows) {
    editorWindow.webContents.setBackgroundThrottling(
